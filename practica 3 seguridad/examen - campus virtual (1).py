...
result = unpad(decipher_aes.decrypt(ciphertext), BLOCK_SIZE)
print(result)

hash_object = SHA256.new(result)
...
descifrado = PKCS1_OAEP.new(private)
datos = descifrado.decrypt(cipherRSA)
...
cipher = DES.new(K_AT, DES.MODE_ECB)
cadena = M.encode("utf-8")
cifrado_K_AT = cipher.encrypt(pad(cadena, BLOCK_SIZE_8))
...
decipher_des = DES.new(K_AT, DES.MODE_ECB)
M = unpad(decipher_des.decrypt(msg_K_AT), BLOCK_SIZE_8).decode("utf-8", "ignore")

cipher = DES.new(K_AT, DES.MODE_ECB)
cadena = M.encode("utf-8")
cifrado_K_BT = cipher.encrypt(pad(cadena, BLOCK_SIZE_8))
...
M = DES.new(K_BT, DES.MODE_ECB)
M = unpad(decipher_des.decrypt(msg_K_BT), BLOCK_SIZE_8).decode("utf-8", "ignore")
...
engineRSACifrado = PKCS1_OAEP.new(public)
Cipher_M = engineRSACifrado.encrypt(M.encode('utf-8'))

h = HMAC.new(K_AB, M.encode('utf-8'), digestmod=SHA256)
HMAC_M = h.digest()
...
descifrado = PKCS1_OAEP.new(private)
Texto_M = descifrado.decrypt(msg_Cipher_M)

h = SHA256.new(Texto_M)  # Ya veremos los hash la semana que viene
signature = pss.new(private).sign(h)
