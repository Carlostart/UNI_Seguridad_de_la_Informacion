...
socket = SOCKET_SIMPLE_TCP('127.0.0.2', 5555)
socket.conectar()
socket.enviar(t_bt.encode('utf-8'))

#-------------------------Mallory-----------------------------------

f = open("mensaje.bin", "wb")
f.write(bytearray.fromhex(t_bt))
...
msg_b_a = hex(int(msg_a_b, 16) -1)[2:]
cipher_aes_b_a = AES.new(t_k_ab, AES.MODE_ECB)
rand2_enc = cipher_aes_b_a.encrypt(pad(msg_b_a.encode("utf-8"),BLOCK_SIZE_AES))

socket.enviar(rand2_enc)
